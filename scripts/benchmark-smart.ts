...
function scoreResult(results: { filePath: string; name: string }[], q: BenchmarkQuery): { top1: boolean; top3: boolean; top5: boolean; rank: number } {
  ...
  for (let j = 0; j < results.length; j++) {
    if (isHit(results[j].filePath, results[j].name, q)) { rank = j + 1; break }
  }
...
async function main() {
  ...
  console.log(`Smart agent benchmark: ${QUERIES.length} queries (${semanticCount} semantic, ${keywordCount} keyword)`)
  console.log(`Database: ${dbPath}\n`)

  const db = new CodeIndex({ dbPath })

  let recallTop1 = 0, recallTop3 = 0, recallMRR = 0, recallLatency = 0
  let grepTop1 = 0, grepTop3 = 0, grepMRR = 0, grepLatency = 0
  let recallWins = 0, grepWins = 0, ties = 0
  ...
  for (let i = 0; i < QUERIES.length; i++) {
    ...
    const recallResults = await db.search(q.query, { limit: 5, includeSnippet: false, mode: q.mode })
    ...
    console.log(`${String(i+1).padStart(2)}  ${q.mode.padEnd(9)} ${rMark}    ${gMark}    ${q.query.slice(0, 48)}...${winner}`)
  }
  ...
  console.log(`Top-1 accuracy:     ${((recallTop1/n)*100).toFixed(0)}% (${recallTop1}/${n})${" ".repeat(12)}${((grepTop1/n)*100).toFixed(0)}% (${grepTop1}/${n})`)
  console.log(`Top-3 recall:       ${((recallTop3/n)*100).toFixed(0)}% (${recallTop3}/${n})${" ".repeat(12)}${((grepTop3/n)*100).toFixed(0)}% (${grepTop3}/${n})`)
  console.log(`MRR:                ${(recallMRR/n).toFixed(3)}${" ".repeat(16)}${(grepMRR/n).toFixed(3)}`)
  console.log(`Avg latency:        ${Math.round(recallLatency/n)}ms${" ".repeat(17)}${Math.round(grepLatency/n)}ms`)
  console.log(`\nHead-to-head:       recall wins ${recallWins}, grep wins ${grepWins}, ties ${ties}`)

  console.log(`\nBy mode (recall):`)
  console.log(`  semantic:  top1=${modeStats.semantic.top1}/${modeStats.semantic.n} (${((modeStats.semantic.top1/modeStats.semantic.n)*100).toFixed(0)}%)  top3=${modeStats.semantic.top3}/${modeStats.semantic.n} (${((modeStats.semantic.top3/modeStats.semantic.n)*100).toFixed(0)}%)`)
  console.log(`  keyword:   top1=${modeStats.keyword.top1}/${modeStats.keyword.n} (${((modeStats.keyword.top1/modeStats.keyword.n)*100).toFixed(0)}%)  top3=${modeStats.keyword.top3}/${modeStats.keyword.n} (${((modeStats.keyword.top3/modeStats.keyword.n)*100).toFixed(0)}%)`)
}

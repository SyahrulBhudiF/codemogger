...
const QUERIES: BenchmarkQuery[] = [
  ...
  // B-tree
  {
    ...
    expectedNames: ["BTreeCursor", "CursorSeekState", "InteriorPageBinarySearchState", "LeafPageBinarySearchState"],
  ...
  },
  {
    ...
    expectedFiles: ["storage/btree.rs"],
    expectedNames: ["payload_overflow_threshold_max", "payload_overflow_threshold_min", "OverwriteCellState"],
...
async function main() {
  ...
  let recallTop1 = 0, recallTop3 = 0, recallTop5 = 0, recallMRR = 0, recallLatency = 0
  // Run grep benchmark
  let grepTop1 = 0, grepTop3 = 0, grepTop5 = 0, grepMRR = 0, grepLatency = 0
  ...
  for (let i = 0; i < QUERIES.length; i++) {
    ...
    // Score recall
    const rTop1 = recallResults.length > 0 && isHit(recallResults[0].filePath, recallResults[0].name, q)
    ...
    for (let j = 0; j < recallResults.length; j++) {
      if (isHit(recallResults[j].filePath, recallResults[j].name, q)) { rRank = j + 1; break }
    }
    if (rTop1) recallTop1++
    if (rTop3) recallTop3++
    if (rTop5) recallTop5++
    ...
    for (let j = 0; j < grepResults.length; j++) {
      if (isCorrectFile(grepResults[j], q.expectedFiles)) { gRank = j + 1; break }
    }
    if (gTop1) grepTop1++
    if (gTop3) grepTop3++
    if (gTop5) grepTop5++
    ...
    console.log(`${String(i+1).padStart(2)}  ${rMark}    ${gMark}    ${q.query.slice(0, 55)}...${winner}`)
  }
  ...
  console.log(`Top-5 recall:       ${((recallTop5/n)*100).toFixed(0)}% (${recallTop5}/${n})${" ".repeat(12)}${((grepTop5/n)*100).toFixed(0)}% (${grepTop5}/${n})`)

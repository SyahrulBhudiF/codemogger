...
function grepSearch(patterns: string[]): string[] {
  ...
  for (const p of patterns) {
    try {
      const out = execSync(`grep -rl "${p}" "${BASE_DIR}" --include="*.rs" 2>/dev/null || true`, { encoding: "utf-8", timeout: 5000 }).trim()
      if (!out) continue
  ...
  return [...hits.entries()].sort((a, b) => b[1] - a[1]).map(([f]) => f).slice(0, 5)
}
...
async function main() {
  ...
  let rTop1 = 0, rTop3 = 0, rMRR = 0, rLat = 0
  let gTop1 = 0, gTop3 = 0, gMRR = 0, gLat = 0
  let rWins = 0, gWins = 0, tieCount = 0
  ...
  for (let i = 0; i < QUERIES.length; i++) {
    ...
    let rRank = 0
    for (let j = 0; j < results.length; j++) { if (isHit(results[j].filePath, results[j].name, q)) { rRank = j + 1; break } }
    ...
    let gRank = 0
    for (let j = 0; j < gResults.length; j++) { if (q.expectedFiles.some(ef => gResults[j].endsWith(ef))) { gRank = j + 1; break } }

    if (r1) rTop1++; if (r3) rTop3++; if (rRank > 0) rMRR += 1 / rRank
    if (g1) gTop1++; if (g3) gTop3++; if (gRank > 0) gMRR += 1 / gRank

    const rv = r1 ? 3 : r3 ? 2 : 0, gv = g1 ? 3 : g3 ? 2 : 0
    if (rv > gv) rWins++; else if (gv > rv) gWins++; else tieCount++

    const ms = q.mode === "semantic" ? mode_s : mode_k
    ms.n++; if (r1) ms.top1++; if (r3) ms.top3++

    const rm = r1 ? "HIT " : r3 ? "top3" : "MISS"
    const gm = g1 ? "HIT " : g3 ? "top3" : "MISS"
    const w = rv > gv ? " ← recall" : gv > rv ? " ← grep" : ""
    console.log(`${String(i+1).padStart(2)}  ${q.mode.padEnd(9)} ${rm}    ${gm}    ${q.query.slice(0, 48)}...${w}`)
  }
  ...
  console.log(`Top-1 accuracy:     ${((rTop1/n)*100).toFixed(0)}% (${rTop1}/${n})${" ".repeat(12)}${((gTop1/n)*100).toFixed(0)}% (${gTop1}/${n})`)
  console.log(`Top-3 recall:       ${((rTop3/n)*100).toFixed(0)}% (${rTop3}/${n})${" ".repeat(12)}${((gTop3/n)*100).toFixed(0)}% (${gTop3}/${n})`)
  console.log(`MRR:                ${(rMRR/n).toFixed(3)}${" ".repeat(16)}${(gMRR/n).toFixed(3)}`)
  console.log(`Avg latency:        ${Math.round(rLat/n)}ms${" ".repeat(17)}${Math.round(gLat/n)}ms`)
  console.log(`\nHead-to-head:       recall wins ${rWins}, grep wins ${gWins}, ties ${tieCount}`)
  console.log(`\nBy mode (recall):`)
  console.log(`  semantic:  top1=${mode_s.top1}/${mode_s.n} (${((mode_s.top1/mode_s.n)*100).toFixed(0)}%)  top3=${mode_s.top3}/${mode_s.n} (${((mode_s.top3/mode_s.n)*100).toFixed(0)}%)`)
  console.log(`  keyword:   top1=${mode_k.top1}/${mode_k.n} (${((mode_k.top1/mode_k.n)*100).toFixed(0)}%)  top3=${mode_k.top3}/${mode_k.n} (${((mode_k.top3/mode_k.n)*100).toFixed(0)}%)`)
}

...
async function main() {
  const N = 500
  const longText = "fn translate_select_plan(plan: &SelectPlan, resolver: &Resolver, program: &mut ProgramBuilder) -> Result<()> { /* ... complex function body with lots of code that exercises the index ... */ }".repeat(3)
  ...
  // Test 1: Table WITHOUT FTS
  {
    ...
    await db.exec("BEGIN")
    const stmt = await db.prepare(`INSERT INTO chunks (file_path, chunk_key, language, kind, name, signature, snippet, start_line, end_line, file_hash, indexed_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    for (let i = 0; i < N; i++) {
      await stmt.run(`file_${i}.rs`, `file_${i}.rs:1:50`, "rust", "function", `func_${i}`, `fn func_${i}()`, longText, 1, 50, "abc123", Date.now())
    }

...
export function formatText(query: string, results: SearchResult[], elapsedMs: number): string {
  if (results.length === 0) {
    return `0 results for "${query}" (${elapsedMs}ms)`
  }

  const lines = [`${results.length} result${results.length === 1 ? "" : "s"} for "${query}" (${elapsedMs}ms)`, ""]
